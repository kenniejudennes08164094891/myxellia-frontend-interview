...
import { getCalendarModal, getOpenedModal } from "@/utils/emmitters";
...
export default function DashboardLayout() {
  ...
  const [isModalOpen, setIsModalOpen] = useState<boolean | null>(false);
    const [isCalendarOpen, setIsCalendarOpen] = useState<boolean | null>(false);

  const openModal = (isOpened: boolean | null) => {
  ...
  useEffect(() => {
    getOpenedModal.subscribe({
      next: (isOpen:boolean | null) => {
        openModal(isOpen);
      }
    });

    getCalendarModal.subscribe({
      next: (openCalendar:boolean | null) => {
        setIsCalendarOpen(openCalendar)
      }
    })
  },[])
